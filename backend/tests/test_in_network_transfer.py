...
def _post(client, body: dict, auth_header) -> "Response":
    """Wrap client.post with the Bearer token derived from body.from_user_id.

    Centralized so every test in this file picks up the auth-required
    contract automatically. Tests that exercise auth failure modes
    bypass this and call client.post directly.
    """
    return client.post(
        "/transfer/in-network",
        json=body,
...
def test_happy_path(client, seed_wallets, wallet_reader, auth_header):
...
def test_sender_wallet_not_found(client, seed_wallets, auth_header):
    bob = seed_wallets["bob"]
    r = _post(client, _build(99999, bob, 100_00), auth_header)
...
def test_receiver_wallet_not_found(client, seed_wallets, auth_header):
    alice = seed_wallets["alice"]
    r = _post(client, _build(alice, 99999, 100_00), auth_header)
    assert r.status_code == 404
...
def test_zero_amount_rejected(client, seed_wallets, auth_header):
    a, b = seed_wallets["alice"], seed_wallets["bob"]
    r = _post(client, _build(a, b, 0), auth_header)
    assert r.status_code == 422


# --------------------------------------------------------------------- 5. amount < 0 rejected

def test_negative_amount_rejected(client, seed_wallets, auth_header):
    a, b = seed_wallets["alice"], seed_wallets["bob"]
    r = _post(client, _build(a, b, -100_00), auth_header)
    assert r.status_code == 422


# --------------------------------------------------------------------- 6. amount > balance

def test_insufficient_balance(client, seed_wallets, wallet_reader, auth_header):
    a, b = seed_wallets["alice"], seed_wallets["bob"]
    r = _post(client, _build(a, b, 99_999_99), auth_header)
...
def test_drain_sender(client, seed_wallets, wallet_reader, auth_header):
    a, b = seed_wallets["alice"], seed_wallets["bob"]
    r = _post(client, _build(a, b, 10_000_00), auth_header)
...
def test_self_transfer_rejected(client, seed_wallets, auth_header):
    a = seed_wallets["alice"]
    r = _post(client, _build(a, a, 100_00), auth_header)
    assert r.status_code == 400
...
def test_idempotency_identical_body(client, seed_wallets, wallet_reader, auth_header):
    a, b = seed_wallets["alice"], seed_wallets["bob"]
    body = _build(a, b, 500_00, key="dup-1")
    r1 = _post(client, body, auth_header)
    r2 = _post(client, body, auth_header)
...
def test_idempotency_different_body_returns_original(client, seed_wallets, wallet_reader, auth_header):
    ...
    r1 = _post(client, _build(a, b, 500_00, key="dup-2"), auth_header)
    assert r1.status_code == 200
    tx1 = r1.json()["data"]["tx_id"]

    r2 = _post(client, _build(a, c, 999_00, key="dup-2"), auth_header)
...
def test_concurrent_split_transfers_respect_balance(client, seed_wallets, wallet_reader, auth_header):
    ...
    def fire(target: int, amount: int) -> int:
        return client.post(
            "/transfer/in-network", json=_build(a, target, amount), headers=headers,
        ).status_code
...
def test_sum_conservation(client, seed_wallets, wallet_reader, auth_header):
    a, b, c = seed_wallets["alice"], seed_wallets["bob"], seed_wallets["carol"]
    before = sum(wallet_reader(x)["balance_kobo"] for x in (a, b, c))
    r = _post(client, _build(a, b, 1_234_00), auth_header)
    assert r.status_code == 200
...
def test_tx_row_shape(client, seed_wallets, auth_header):
    import importlib
    ...
    a, b = seed_wallets["alice"], seed_wallets["bob"]
    r = _post(client, _build(a, b, 500_00), auth_header)
...
def test_non_integer_amount_rejected(client, seed_wallets, auth_header):
    a, b = seed_wallets["alice"], seed_wallets["bob"]
    r = client.post(
        "/transfer/in-network",
...
def test_string_amount_rejected(client, seed_wallets, auth_header):
    a, b = seed_wallets["alice"], seed_wallets["bob"]
    r = client.post(
        "/transfer/in-network",
...
def test_missing_idempotency_key_rejected(client, seed_wallets, auth_header):
    a, b = seed_wallets["alice"], seed_wallets["bob"]
    r = client.post(
        "/transfer/in-network",
...
def test_idempotency_key_too_long(client, seed_wallets, auth_header):
    a, b = seed_wallets["alice"], seed_wallets["bob"]
    r = _post(client, _build(a, b, 100_00, key="x" * 200), auth_header)
    assert r.status_code == 422


# --------------------------------------------------------------------- 18. wallet version bumps

def test_wallet_version_bumps(client, seed_wallets, wallet_reader, auth_header):
    a, b = seed_wallets["alice"], seed_wallets["bob"]
...
def test_settled_at_present(client, seed_wallets, auth_header):
    a, b = seed_wallets["alice"], seed_wallets["bob"]
    r = _post(client, _build(a, b, 100_00), auth_header)
...
def test_high_concurrency_no_oversend(client, seed_wallets, wallet_reader, auth_header):
